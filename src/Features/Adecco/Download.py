...
for url in urls:
    ...
    title = url.replace("https://adecco.com.vn/en/jobs/detail/", "")  
    title = title[:-9].replace("-", " ").title().replace(" ", "") 

    with open(f"{o_fol}\\Webpage @{title} #{timestamp} .html", mode="w", encoding="utf-8-sig") as file:
